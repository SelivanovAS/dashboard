...
def classify_appellant_role(
    ...
    «Заявитель» даёт не имя, а слово-роль («ИСТЕЦ» / «ОТВЕТЧИК»). Такой
    вход распознаём напрямую до токенного матчинга (иначе он не совпадёт
    ни с именем истца, ни ответчика и ложно уйдёт в «Иное лицо»).
    """
    ...
    _bare_role = {
        ...
        "иное лицо": "Иное лицо",
    }.get(appellant_raw.strip().lower())
    if _bare_role:
        return (_bare_role, _bare_role)
