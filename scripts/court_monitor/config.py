...
# backfill_fi_links: если целевой поиск по номеру дело не нашёл, повторная
# попытка — не раньше чем через столько дней (штамп fi.link_backfill_checked_at).
# Без штампа ненаходимое дело жгло HTTP + WARNING каждый прогон (13-228/2026,
# Урал, 12.08.2026). Не одноразово, как appeal_appellant_checked_at: карточка
# может появиться в выдаче позже (суд публикует с лагом).
LINK_BACKFILL_RETRY_DAYS = 7
