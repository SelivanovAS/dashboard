...
| `announce_imported_appeal_cases` (дела из дампа апелляции → «Новые дела», один раз) | [scripts/court_monitor/runs.py:1937](scripts/court_monitor/runs.py:1937) |
...
| `fi_court_by_domain` (суд 1-й инст. по (домен, srv_num); None = чужой регион) | [scripts/court_monitor/courts.py:238](scripts/court_monitor/courts.py:238) |
...
| `bank_writ_expected` (ждём ли ИЛ: отказ/присоединение → нет) | [scripts/court_monitor/lifecycle.py:1318](scripts/court_monitor/lifecycle.py:1318) |
...
| `repair_vacated_default_judgments` (ремонт: откат решения + возврат в трек) | [scripts/court_monitor/lifecycle.py:2043](scripts/court_monitor/lifecycle.py:2043) |
| `intake_bank_rows` (блок 3b: приём исков банка с выдачи в прогоне) | [scripts/court_monitor/runs.py:1965](scripts/court_monitor/runs.py:1965) |
...
| `bank_track_pending` (гейт раскладки 7c — по данным, не по счётчику загрузки) | [scripts/court_monitor/runs.py:2074](scripts/court_monitor/runs.py:2074) |
...
| `appeal_court_by_domain` (апел-суд по appeal.court_domain) | [scripts/court_monitor/courts.py:151](scripts/court_monitor/courts.py:151) |
| `appeal_court_for_fi_domain` (апел-суд по домену суда 1-й инст.) | [scripts/court_monitor/courts.py:178](scripts/court_monitor/courts.py:178) |
...
| `migrate_appeal_court_fields` (бэкфилл суда в блоках appeal) | [scripts/court_monitor/lifecycle.py:2013](scripts/court_monitor/lifecycle.py:2013) |
...
| `looks_like_outage_page` (URL-независимый детект заглушки — канарейка) | [scripts/court_monitor/parsing/search.py:451](scripts/court_monitor/parsing/search.py:451) |
...
| `update_parse_health` — детектор молчаливой поломки парсеров | [scripts/court_monitor/health.py:118](scripts/court_monitor/health.py:118) |
| `advance_case_stage` / `is_case_archived` / `migrate_stages` | [scripts/court_monitor/lifecycle.py:2155](scripts/court_monitor/lifecycle.py:2155) |
...
| `main_json` (оркестрация полного прогона) | [scripts/court_monitor/runs.py:2382](scripts/court_monitor/runs.py:2382) |
...
- **Ритм опроса** (`should_skip_case`): до решения — обычный smart-skip; после
  ...
  ([lifecycle.py:2988](scripts/court_monitor/lifecycle.py:2988)) стоит ПЕРВОЙ и
...
- **Сводка** — `GET /admin/visits` (`handleAdminVisits`, [worker.js:2603](cloudflare-worker/worker.js:2603)), **только владельцу**: читает историю ОДНИМ `list` по префиксу и **без единого `get`** (`metadata` приходит вместе с ключами), пагинация по курсору ОБЯЗАТЕЛЬНА (бескурсорный `handleAdminImportLog` копировать нельзя — при >1000 ключей молча теряется хвост). Наружу уходит только огрызок `vid` (6 символов). ⚠️ Поллинга НЕТ и оператору не отдаётся: lists на free-tier тоже 1000/день, а операторов на Урале ~14.
...
- Фронт — «один тап» (29.08.2026): в настройках ОДНА умная кнопка
  ...
  сервисный ряд — «Скопировать ссылку» и «Скачать файл (.ics)» всегда (обе
  добывают токен сами), «Перевыпустить» и показ ссылки в `<details>` — только
  при выданном токене. **Корпоративный OWA (03.09.2026):** рабочие станции
...
«Обновить»), на 320px они жались до 46px. Синк, push и календарь — вещи
«настроил один раз», им место за шестерёнкой; **в шапке остались ⚙, тема и
«Обновить»** (ровно три `<button>`, страж `test_header_has_gear_and_only_three_capsules`).
...
- **Разделы сверху вниз:** «уведомления» (`settingsPushSectionHtml` —
  состояние словами: `on`/`ready` + кнопка `#st-push-btn` → `settingsPushEnable`/
  ...
  «календарь заседаний» (`calFeedBlockHtml`, см. раздел выше),
  «о приложении» (`settingsAboutSectionHtml`: версия из `?v=` у app.js,
  территория из `#header-region`, «Показать «Что нового»» → `showWhatsNewAgain`
  снимает `lsKey('whatsnew_seen')`).
...
- **Анонс «Что нового»** — `WHATSNEW_ID = 'settings-2026-09'`: переезд
  настроек (кнопка `whatsNewOpenSettings`) + «Скачать файл» для OWA; прежний
