...
Исходный SHA-256: `0f3d545005c7db2514b30f19ac31069073db9786b199ba30c01db2e735a4278d`.
...
**С 15.07.2026 система регионализована** (тиражирование на территории Уральского банка, этап 1 — Свердловская обл.+ЯНАО): регион = конфиг в `scripts/court_monitor/regions/` (выбор — env `REGION`, дефолт hmao), территория = форк, отличающийся только Variables/секретами и тремя «файлами территории» (`region_front.js` — обязательный `STORAGE_NS` (неймспейс localStorage: фронты на одном origin github.io, без него звёзды/заметки перемешиваются между территориями; эталон NS не задаёт), `manifest.json`, `wrangler.toml`). Апелляций в регионе может быть НЕСКОЛЬКО (`APPEAL_COURTS`, `appeal.court_domain` в JSON, составной ключ связки). **С 25.08.2026 проверочный код бывает и на АПЕЛЛЯЦИИ** (Свердловский облсуд): `search_gated` у неё значит «код ожидаем» — поиск прогон делает по-прежнему (снимут код — вернётся сам), но капча не поднимает 🩺-алерт и пишется в журнал здоровья как «не наблюдали»; дела заводит дамп выдачи, облсуд закреплён ПЕРВОЙ строкой списка судов админки и виден всем операторам независимо от их набора «моих судов». **С 28.08.2026 у облсуда мягкий режим отменён** (решение юриста): второй флаг `CourtConfig.search_disabled=True` — поиск не делается ВОВСЕ (ни HTTP, ни записи в журнал здоровья: мягкий гейт писал None, а детектор считал None HTTP-фейлом — «страница поиска не загружается 16 прогонов подряд» каждый слот); ветка ПЕРВОЙ в цикле фазы 3, сама кладёт домен в `appeal_search_gated_now` (гейт дослинка), `search_gated` оставлен (дампы/бэкфиллы/админка), карточки мониторятся; застывший ключ `appeal:oblsud--svd` из parse_health.json форка удалён разово; стражи — `TestSearchDisabled`. **С 04.09.2026 под кодом и Суд ХМАО-Югры** (`appeal:oblsud` 21→0 семь слотов, `captcha_search`; карточки читались 31/31) — режим как у СВД (оба флага в `regions/hmao.py`, решение юриста), дампы вставляет сам юрист под owner (`OPERATOR_SECRET` у ХМАО не задан); блок `region` в cases.json при смене флагов публикуется ТОЧЕЧНО (`updated_at` не трогать), застывший ключ `appeal:oblsud` из parse_health.json удалён разово. **Разбор того же дня вскрыл, что 🩺 «Мониторинг парсеров» не доходил до Telegram с флипа на Mac/VPS (19.08/28.08)**: шлёт его Python `send_telegram`, а у прогона на VPS токена нет (дать нельзя — `digest_will_deliver = bool(TELEGRAM_BOT_TOKEN)`, каждый слот слал бы дайджест), replay блок 4e не выполняет. Теперь строки детектора лежат в `last_run.alerts` журнала здоровья, а `parse_and_push.sh` ретранслирует их shell-каналом (`cloud_run_ok.py --health-alerts` → `cm_alert_telegram … 🩺`, только сегодняшний `last_run.at`, дедуп по строке за день в `.runtime/health_alerts_sent.<дата>`); карточные счётчики 4e в ретрансляцию НЕ входят — они едут 🚨-алертом прогресса. Капча на поиске ЛЮБОГО непомеченного суда (три инстанции, `health_captcha` {ключ журнала: домен}) ведётся капча-состоянием `update_parse_health` (поля `captcha_since`/`captcha_alerted_on`): 🔐-алерт с рецептом при первом обнаружении, напоминание раз в день, ✅ при снятии, текст «поиск вернул 0» для такого ключа глушится; прежний повторявшийся каждый прогон `fi_challenge` удалён. Стражи — `TestCaptchaHealthWiring`, `TestHealthAlertsRelay`, капча-тесты в `TestUpdateParseHealth`. Тем же днём починен фолбэк суда у CSV-строки апелляции без JSON-двойника (`_appeal_domain` не переживает round-trip через CSV): суд теперь выводится из суда 1-й инстанции строки (`match_fi_court_by_short_name` → `appeal_court_for_fi_domain`), а не «первый апел-суд региона» — прежний фолбэк качал карточку ЯНАО-дела 33-2042/2026 со Свердловского ОБЛСУДА (чужой суд → страница на 4 таблицы → degraded без бампа `last_checked_at` → вечный ретрай каждым слотом; для ХМАО все ветки байт-в-байт — апел-суд один). **С 04.09.2026 — кассация в ПРЕЗИДИУМЕ облсуда (дела мировых судей, ГПК с 05.2026):** третий кассационный источник — `RegionConfig.presidium_courts` (`regions/hmao.py` — «Президиум Суда ХМАО-Югры», `sverdlovsk_yanao.py` — президиумы СВД и ЯНАО; `court_type="cassation"`, `delo_id=2800001`, домен апел-суда, `search_gated`+`search_disabled` — поиск раздела за проверочным кодом), КСОЮ (`cassation_court`) остаётся один. Какой суд у блока `cassation` — решает `cassation_court_by_domain(court_domain)` (courts.py): фаза 4d перечитывает карточку с домена БЛОКА (жёсткий `CASSATION_COURT` откатывал бы дело президиума на 7kas), ссылки дайджеста — `cassation_card_url`, фронт — `cs.court_domain`/`cs.delo_id`. Новые дела заводит ТОЛЬКО дамп выдачи раздела 2800001: **раздел выбирает сам дамп** (`resolve_court(domain, delo_id=)` по `delo_id` из ссылок карточек — на домене облсуда живут апелляция и президиум, оператор шлёт голый домен; Worker/workflow/очередь VPS без правок), ветка `_main_presidium` → `import_presidium_rows` (карточка обязательна; строки с датой поступления до `PRESIDIUM_DUMP_SINCE`=01.05.2026 — `[SKIPPED OLD]`, счётчик `skipped_old`: выдача по «Сбербанк» тянет 22 из 25 дел 2019 года; `[NO SBER]` → `subsidiary`) → боевой `link_cassation_cases` (discovery: **id = номер президиума «4Г-…»** — решение юриста, стаб 1-й инст. `magistrate: true` с номером мирового судьи, `notes` «Заведено дампом президиума», блок `import.source="dump_presidium"` → `announce_imported_presidium_cases` объявляет один раз в «📥 Новые касс. дела»; `announce_imported_cases` такие дела пропускает). Ключ кассации везде — ПАРА (домен, номер): «4Г-N/YYYY» двух президиумов Урала совпадают (`_cass_key`, `dedupe_cassation_by_internal_number`); находка президиума по FI-номеру не матчится (мирового судью не мониторим), а `collect_existing_ids`/`collect_fi_dedup_index` номера с `magistrate` не индексируют — иначе wildcard заблокировал бы районное дело с тем же номером. Парсер: `_CASS_INTERNAL_NUM_RE` = `\d+Г-…` (8Г и 4Г), «[44Г-2/2026]» в скобках → `cassation_number`, «Судья (мировой судья) первой инстанции: ФИО (участок)» → `fi_judge`+`fi_court_long` «Мировой судья (участок)»+`fi_magistrate` (`split_magistrate_judge`), номер с заголовка карточки → `page_case_number`, `determine_bank_role_from_participants(synonyms=True)` — ВЗЫСКАТЕЛЬ→Истец/ДОЛЖНИК→Ответчик только для кассации, исходы «СУДЕБНЫЙ ПРИКАЗ ОТМЕНЕН» → reversed, «…с направлением на новое рассмотрение» → remanded. Стадии: remanded у президиума НЕ уводит в `awaiting_relink` (нижестоящих не мониторим), архив по общим окнам (`is_presidium_cassation`). Админка: третья закреплённая строка « — президиум (кассация)», ключ `domain|srv|cassation`, автоопределение по `delo_id` дампа, `section` в отчёте (jq → Worker → админка). Фронт: favor в кассации — по кассатору (`cassAppellantIsBank`). Стражи — `test_presidium_cassation.py`, `test_import_presidium_dump.py`. **Иск, не принятый к производству, не заводится (14.08.2026):** строка «банк-ответчик» с итогом возврата / отказа в принятии / передачи по подсудности отсеивается гейтом `fi_not_accepted_kind` (lifecycle.py) ДО проверки ссылки и до `_fetch_main_card` — маркер `[NOT ACCEPTED]`, счётчик `not_accepted` (до оператора едет теми же тремя звеньями). Фильтр итогов был только у трека исков БАНКА (`_EXCLUDED_RESULT_RX`), и возврат любой давности заводился активным, объявлялся «новым иском» и 60 дней занимал картотеку, каждый прогон качая карточку (19 дел Урала, 6 ХМАО — разбор юриста). Класс узкий: прекращение и «оставлено без рассмотрения» заводятся по-прежнему (производство было, по частной жалобе дело оживает под ТЕМ ЖЕ номером), присоединение — тоже (дело живёт под номером приёмника). Зеркало гейта — блок 3 `main_json` (отсев ДО `_discovered_already_resolved_old`, стережёт `TestNotAcceptedIntakeWiring`); точечное добавление из админки гейта НЕ имеет осознанно — номер туда вводит юрист вручную. Уже заведённые 25 дел убраны разово. Подробно — [docs/Тиражирование_регионы.md](../../../docs/Тиражирование_регионы.md).
...
- `scripts/court_monitor/` — **пакет модулей** (читать только нужный):
  ...
  - [appeal_intake.py](../../../scripts/court_monitor/appeal_intake.py) — конвертеры строки выдачи апелляции (карточка → строка → JSON-дело → обратно для анонса). Зеркало `bank_intake`: общий модуль для ДВУХ каналов ввода — поиск апелляции в прогоне и дамп капчёвого апел-суда (`scripts/*.py` не имеют права тянуть runs.py целиком). runs.py ре-экспортирует прежние приватные имена.
  - [digest/](../../../scripts/court_monitor/digest/__init__.py) — `llm.py` (Claude/GigaChat/OpenRouter — выбор через `LLM_PROVIDER`; промпты — патч-цели тестов живут тут; **с 28.08.2026 у пересказов актов фолбэк-ПРОВАЙДЕР**: openrouter исчерпал и «модель дня», и `openrouter/free` → одна попытка боевого Claude при живом `ANTHROPIC_API_KEY` (`LLM_SUMMARY_PROVIDER_FALLBACK=0` — выключить; счётчик `llm_summary_provider_fallback_saved`, «спасено Claude N» в сводке) — иначе пересказ терялся НАВСЕГДА: акт объявляется один раз, и сырой отрывок замерзал в дайджесте и «AI анализе» (инцидент 28.08.2026, Урал: оба акта выпуска при живом ключе Claude в env replay); на Mac-резерве ветка недостижима — `missing_llm_key_name` отсекает раньше), `postprocess.py` (валидация/чистка HTML), `template.py` (программный рендер — **боевой путь с 03.07.2026**, компакт-вёрстка без отступов), `core.py` (диспетчер `generate_digest`), `lint.py` (программный линтер готового HTML после отправки: полнота номеров, счётчики (N), теги, футер → 🩺-алерт; `DIGEST_LINT=0` — выключатель). Рядом с линтером (и ТОЛЬКО там — блок 4e идёт ДО генерации дайджеста, счётчик в нём всегда 0) с 02.08.2026 стоит `_alert_llm_summary_failures`: при `METRICS["llm_summary_failed"] > 0` шлёт 🩺-алерт «пересказы актов» — иначе отказ провайдера (429 free-пула OpenRouter, 17.07.2026: в дайджест уходила сырая мотивировка) был виден только в логе прогона. Зовётся из `main_json` **и с 21.08.2026 из `main_replay_last`**: с флипом на Mac-резерв (19.08) боевой дайджест делает replay, и алерт, живший только в `main_json`, срабатывал лишь на Mac — где Telegram-токена нет и сообщение никуда не уходило, а настоящий отказ OpenRouter не заметил бы никто. Тем же днём «ключа у провайдера нет вовсе» перестало считаться сбоем (`llm.missing_llm_key_name` — единственное место, где живёт соответствие «провайдер → его ключ», им же пользуется `validate_environment`): `summarize_act_motivation` выходит ДО вызова, растит `llm_summary_skipped_no_key` вместо `llm_summary_failed` и печатает одну строку за процесс — иначе Mac каждое утро рапортовал «сбоев 6 из 6» о несуществующем 429. ⚠️ Гард стоит ПОСЛЕ проверки кэша: пересказ, оплаченный replay'ем и закоммиченный в `.act_summaries.json`, обязан отдаваться и на машине без ключей. Прод — гибрид: события рендерит код, LLM только пересказывает мотивировки актов; `DIGEST_FULL_LLM=1` — откат на полный LLM-дайджест.
...
- `data/parse_health.json` — журнал здоровья парсеров: пер-источник история количества результатов поиска (20 судов 1-й инст., апелляция, 7kas до/после HMAO-фильтра). Для судов 1-й инст. счётчик — сберовские строки ДО фильтра ролей (`stats["sber_rows"]` из `parse_first_instance_search`; с 31.07.2026 прогон зовёт парсер с `keep_all_roles=True` и делит строки по роли сам — метрика от этого не изменилась): вал исков самого банка вытесняет ответчик-дела со стр. 1 и обнулял бы метрику без поломки (ложный алерт по Октябрьскому р/с 14–15.07.2026). Детектор «молчаливой поломки» (`update_parse_health`, блок 4e в `main_json`) шлёт сервисный 🩺-алерт в Telegram: суд с медианой ≥1 вернул 0 (на 1-м и 3-м нулевом прогоне + сообщение о восстановлении), HTTP-фейл 3 прогона подряд, все источники разом по нулям, ≥5 карточек-«огрызков» за прогон. **С 04.09.2026 — капча-состояние источника** (kwarg `captcha` = {ключ: домен} тех, чей поиск пришёл проверочным кодом; поля `captcha_since`/`captcha_alerted_on`): 🔐-алерт с рецептом (что править в `regions/<регион>.py`) при первом обнаружении, напоминание раз в день, ✅ «код снят» при первой странице без кода, дубли «вернул 0»/«снова отдаёт» для такого ключа глушатся; `zero_streak`/`alerted_zero` при этом ведутся — точка в админке красная. Строки детектора этого прогона дублируются в `last_run.alerts` — ретрансляция с VPS/Mac (см. «Автозапуск»).
...
| `match_hmao_first_instance` (длинная форма → CourtConfig) | [scripts/court_monitor/courts.py:111](../../../scripts/court_monitor/courts.py:111) |
| `RegionConfig` (регион-конфиг: суды, маркеры, public_info) | [scripts/court_monitor/regions/base.py:179](../../../scripts/court_monitor/regions/base.py:179) |
| `CourtConfig.search_gated` (капча: у 1-й инст. поиск ВЫКЛ; у апелляции — «код ожидаем»: поиск идёт, но капча не авария) | [scripts/court_monitor/regions/base.py:39](../../../scripts/court_monitor/regions/base.py:39) |
| `CourtConfig.search_disabled` (жёсткий выключатель поиска апелляции: ни HTTP, ни журнала здоровья; облсуд СВД с 28.08.2026) | [scripts/court_monitor/regions/base.py:45](../../../scripts/court_monitor/regions/base.py:45) |
| `presidium_court_by_domain` / `cassation_court_by_domain` / `cassation_card_url` (суд блока `cassation`: президиум облсуда или КСОЮ — фаза 4d, дайджест) | [scripts/court_monitor/courts.py](../../../scripts/court_monitor/courts.py) |
| `_main_presidium` / `import_presidium_rows` (ветка дампа президиума; `resolve_court(domain, delo_id)` — раздел по дампу) | [scripts/import_search_dump.py](../../../scripts/import_search_dump.py) |
| `split_magistrate_judge` (ФИО мирового судьи + участок → судья / «Мировой судья (участок)») | [scripts/court_monitor/parsing/cassation.py](../../../scripts/court_monitor/parsing/cassation.py) |
| `appeal_intake.py` (конвертеры строки выдачи апелляции: прогон + дамп) | [scripts/court_monitor/appeal_intake.py:25](../../../scripts/court_monitor/appeal_intake.py:25) |
| `announce_imported_appeal_cases` (дела из дампа апелляции → «Новые дела», один раз) | [scripts/court_monitor/runs.py:1969](../../../scripts/court_monitor/runs.py:1969) |
| `courts_for_search` (суды автопоиска: enabled и не gated) | [scripts/court_monitor/courts.py:48](../../../scripts/court_monitor/courts.py:48) |
...
| `fi_court_by_domain` (суд 1-й инст. по (домен, srv_num); None = чужой регион) | [scripts/court_monitor/courts.py:243](../../../scripts/court_monitor/courts.py:243) |
...
| `repair_cancelled_merges` (объединение отменили → снять флаги) | [scripts/court_monitor/lifecycle.py:520](../../../scripts/court_monitor/lifecycle.py:520) |
| `resolve_bank_merged_targets` (подбор дела-приёмника по ФИО ответчика) | [scripts/court_monitor/linking.py:1568](../../../scripts/court_monitor/linking.py:1568) |
...
| `case_court_key` / `dedupe_new_archive_entries` (ключ (домен, id) — номера не уникальны между судами) | [scripts/court_monitor/linking.py:1503](../../../scripts/court_monitor/linking.py:1503) |
...
| `match_region_first_instance` (обобщённый матчер по региону) | [scripts/court_monitor/courts.py:63](../../../scripts/court_monitor/courts.py:63) |
| `appeal_court_by_domain` (апел-суд по appeal.court_domain) | [scripts/court_monitor/courts.py:156](../../../scripts/court_monitor/courts.py:156) |
| `appeal_court_for_fi_domain` (апел-суд по домену суда 1-й инст.) | [scripts/court_monitor/courts.py:183](../../../scripts/court_monitor/courts.py:183) |
...
| `card_breaker_allows` (пер-суд предохранитель карточек: гейт пропуск/проба) | [scripts/court_monitor/netutil.py:712](../../../scripts/court_monitor/netutil.py:712) |
| `looks_like_outage_page` (URL-независимый детект заглушки — канарейка) | [scripts/court_monitor/parsing/search.py:463](../../../scripts/court_monitor/parsing/search.py:463) |
...
| `rotate_cold_archive` (горячий → холодный архив) | [scripts/court_monitor/linking.py:1250](../../../scripts/court_monitor/linking.py:1250) |
...
| `parse_cassation_search_page` — поиск 7kas (HMAO-фильтр) | [scripts/court_monitor/parsing/cassation.py:85](../../../scripts/court_monitor/parsing/cassation.py:85) |
| `classify_cassation_outcome` — детерм. enum исхода | [scripts/court_monitor/parsing/cassation.py:240](../../../scripts/court_monitor/parsing/cassation.py:240) |
| `_extract_cassation_act_text` (секция `cont_doc1`) + `parse_cassation_card` | [scripts/court_monitor/parsing/cassation.py:438](../../../scripts/court_monitor/parsing/cassation.py:438) |
| `relink_awaiting_relink_first_instance` (re-link после remanded) | [scripts/court_monitor/linking.py:235](../../../scripts/court_monitor/linking.py:235) |
| `link_cases` (FI ↔ апелляция) | [scripts/court_monitor/linking.py:55](../../../scripts/court_monitor/linking.py:55) |
...
| `update_active_cases` (обход карточек активных дел) | [scripts/court_monitor/runs.py:592](../../../scripts/court_monitor/runs.py:592) |
| `main_json` (оркестрация полного прогона) | [scripts/court_monitor/runs.py:2422](../../../scripts/court_monitor/runs.py:2422) |
...
| `polish_digest_html` — LLM-полировщик (опц.) | [scripts/court_monitor/digest/llm.py:1261](../../../scripts/court_monitor/digest/llm.py:1261) |
...
| Claude model: `claude-haiku-4-5-20251001` (`_current_digest_model_name`) | [scripts/court_monitor/digest/llm.py:1402](../../../scripts/court_monitor/digest/llm.py:1402) |
  ...
  "cases": [
    {
      ...
      "first_instance": {
         ...
         "magistrate",             // true у стаба дела МИРОВОГО судьи (кассация президиума,
                                   // 04.09.2026): суд не мониторим, номер в индексы дедупа не идёт
      ...
      "cassation":      { "case_number", "cassation_number", "court", "court_domain", "delo_id",  // суд кассации: КСОЮ или президиум облсуда (04.09.2026)
                          "judge",
...
> 🖥 **С 28.08.2026 боевой парсинг — на VPS Cloud.ru** (195.19.66.234,
> Ubuntu 24.04, egress РФ; проба 27.08 — обе территории зелёные, RAM ≤81 МБ
> из 960). Работают ТЕ ЖЕ Mac-скрипты через тонкие шимы
> [ops/vps-run/](../../../ops/vps-run/README.md) (`parse_all.sh`/`import_all.sh` →
> exec боевых `ops/mac-local-run/*` с `--anywhere`; заглушка `shims/netstat`
> = «не в сети Сбера», `CM_COURT_ROUTES_READY=1` выключает маршруты Сбера) —
> своей копии логики у VPS НЕТ, вся механика ниже (слоты, окно 08:45,
> транзакции, дочитка, очередь импортов) описывает и его. Слоты — systemd-
> таймеры `court-parse.timer`/`court-import.timer` (зеркала plist, стражи —
> [scripts/tests/test_vps_run.py](../../../scripts/tests/test_vps_run.py)); сервер в
> TZ Asia/Yekaterinburg (`vps_env.sh` жёстко требует +0500); git push — два
> deploy-ключа, выбор по cwd в `~/.ssh/config` (Match exec); секреты — в
> `~/.config/court-monitor/` на VPS. **Mac-агенты ВЫГРУЖЕНЫ 28.08.2026**
> (`launchctl unload` обоих plist; файлы на месте) — Mac теперь ручной
> резерв: реактивация `launchctl load`, разовый прогон — пульт. ⚠️
> Одновременно VPS и Mac работать не должны: гейт «один дайджест в день»
> межхостовой гонки не ловит (два marker-коммита = два дайджеста). Алерты с
> VPS подписаны «Mac-парсинг (клон)» — префикс жёсткий в боевом скрипте.
> **🩺-алерты здоровья парсеров с VPS/Mac (04.09.2026)**: Python здесь без
> `TELEGRAM_BOT_TOKEN` (с токеном каждый слот слал бы дайджест —
> `digest_will_deliver`), поэтому `send_telegram` блока 4e — no-op, а replay
> 4e не выполняет: с 19.08 ни одно 🩺 не доходило. Теперь `main_json` кладёт
> строки детектора в `last_run.alerts`, `parse_and_push.sh` сразу после
> парсинга зовёт `cloud_run_ok.py --health-alerts` (только сегодняшний
> `last_run.at`) и шлёт новые строки `cm_alert_telegram … "🩺"` с префиксом
> «Мониторинг парсеров (клон)»; дедуп по строке за день —
> `.runtime/health_alerts_sent.<дата>`. Карточные счётчики (не прочитано /
> предохранитель) в ретрансляцию не входят — они уже в 🚨-алерте прогресса.
...
> юриста), расписание с 21.08.2026 — **слоты 06:00–08:30 каждые 30 мин +
> доставочный 08:45** (он же ПОСЛЕДНИЙ: слот 09:00 юрист велел убрать,
> страховки после окна нет — упавший доставочный слот даёт алерт и ручной
> запуск с пульта). ⚠️ Слоты — точки, где launchd МОЖЕТ начать, а не
> гарантия запуска каждые 30 мин: пока агент работает, наступивший интервал
> пропускается без очереди (замер 21.08.2026: 08:00→09:02 обе территории,
> слоты 08:30 и 09:00 не запускались, следующий пуск 09:30). Прогон двух
> территорий ~60 мин, поэтому идущий в 08:45 прогон доставляет САМ — окно
> проверяется по времени ФИНИША, в конце скрипта. НЕПОЛНАЯ попытка (поиски слепые
...
> формулировки побайтово. **С 26.08.2026 дочитка покрывает и ПОИСКИ**: суд,
> чья выдача сегодня уже дала строки (`searched_ok_today` в health.py по
> `sources` журнала: `last_run_at` сегодня + `last_count > 0` + без
> `fail_streak`), повторный слот не запрашивает — гейт `search_skip_keys`
> стоит во всех трёх поисковых фазах `main_json` (7kas / апелляция / 20 судов
> 1-й инст.) под тем же `SKIP_CHECKED_TODAY`. Критерий строгий, «>0 строк»:
> WAF-заглушка парсится как «0 строк» и успехом не считается — слепой поиск
> ретраится каждым слотом (26.08 попытки 1–5 ХМАО были слепыми, зрячей стала
> шестая); суды с честным нулём опрашиваются каждый слот (цена осознанная),
> капчёвая апелляция (`search_gated`, пишет None) не пропускается никогда —
> «снимут код — вернётся сам». Пропущенный источник НЕ попадает в
> `health_obs`, его `last_run_at`/`counts` остаются от удачного слота —
> дневной union `cloud_run_ok` цел без правок; вместе с поиском слот
> пропускает и его пассажиров (промоушен М→2 по строке, new_fi, авто-подхват
> 3b, канарейку предохранителя) — всё отработало в удачном слоте. Стражи —
> [scripts/tests/test_search_skip_today.py](../../../scripts/tests/test_search_skip_today.py).
> **Доставку решает ОКНО, а не вердикт** (21.08.2026,
> «дайджест не раньше 08:45»): первый слот, ЗАВЕРШИВШИЙСЯ после
> `DELIVERY_WINDOW_MIN`=525 (08:45), или --force → `--mark-delivered` +
> маркерный коммит «(Mac-парсинг)» → replay шлёт ОДИН дайджест со всем
> накопленным с 06:00; при мёртвых канарейках в окне накопленное
> отправляется доставочным коммитом без парсинга. ⚠️ `RUN_OK` на доставку
...
> (это не гарантия семантически валидной страницы). **С 25.08.2026** preflight
> проверяет реальные search/card `sud_delo`, пишет VPN/маршрут/egress ID в
> `network_fingerprint.json`, а telemetry хранит все попытки текущего дня и
> union `planned_case_ids_today` / `read_case_ids_today` по инстанциям;
> `cloud_run_ok` показывает FI / апелляцию / кассацию раздельно. Общий мягкий
> дедлайн прогона — 3300 с: новые HTTP прекращаются, прочитанное сохраняется.
> Гейт слота —
...
- **Дайджест-на-push:** [.github/workflows/replay_on_push.yml](../../../.github/workflows/replay_on_push.yml)
  ...
  **С 26.08.2026 web push не обгоняет сайт**: replay-шаг идёт с
  `DEFER_WEB_PUSH=1` (Telegram уходит сразу — полный текст в сообщении),
  workflow коммитит `last_digest.json`, ждёт до ~7 мин, пока Pages отдаст
  байты нашего файла (сравнение sha256 по публичному URL, НЕ деплой-API —
  тот 26.08 показывал cancelled-сборки; URL из `github.repository_owner`,
  не хардкодить — workflow общий с форками), и шлёт пуш отдельным шагом
  `--push-web-only` (`main_push_web_only` в runs.py: тот же контекст +
  эхо-фильтр + пер-подписочная персонализация, дайджест НЕ перегенерируется).
  Журнал `last_personal_pushes.json` докоммичивается ПОСЛЕ отправки —
  иначе админка читала бы вчерашнюю рассылку. Причина: очередь Pages из
  трёх пушей доставочного слота задержала публикацию на ~5 мин, и клик по
  пушу в 08:55 открывал вчерашний дайджест. ⚠️ Откат — снимать ПАРОЙ:
  `DEFER_WEB_PUSH` и оба шага ожидания/отправки, иначе двойной пуш.
  Стражи — [scripts/tests/test_replay_push_after_pages.py](../../../scripts/tests/test_replay_push_after_pages.py).
...
4. Mac уже разбужен (18.08.2026): агенты `com.court-monitor.parse` (будни
   06:00–08:30 каждые 30 минут + 08:45) и `com.court-monitor.import` (будни 12:00–20:00 каждые
...
⚠️ **С 08.09.2026 VPS — ОСНОВНОЙ исполнитель операторских импортов, а не дочитка** (решение
юриста): Worker с `IMPORT_EXECUTOR = "vps"` (`[vars]` wrangler.toml, оба Worker'а) дамп выдачи и
точечную пачку в GitHub НЕ диспатчит — кладёт в KV и заводит запись журнала со статусом
`queued`; `import_queue.jq` берёт `queued` СРАЗУ, без грейсов (живого облачного джоба нет), по
слотам `court-import.timer` **будни 12:00/14:00/16:00/18:00/20:00** (+ утренние `run_imports`
после парсинга). Причина: раннер GitHub — лотерея адресов, и провалы всё равно доделывал VPS;
теперь писатель один. Цена (осознанная): дамп в 12:05 обработается в 14:00. Слоты продублированы
в `IMPORT_SLOTS_LOCAL` (страж `test_slots_var_mirrors_timer`) — Worker отдаёт `next_slot_at`
в ответе POST и `slots` в `/admin/import-log`: страница в режиме vps **не поллит журнал** (каждый
тик — KV list, а ждать до двух часов), а пишет «сервер обработает сегодня в 14:00», форму чистит
сразу; запись `queued` старше последнего слота помечается «⚠ сервер не забрал — проверьте VPS».
**С 09.09.2026 — НЕМЕДЛЕННАЯ попытка** (решение юриста «пробовать сразу, провалы — в окно»): Worker в
ветке vps пишет ещё один KV-ключ `import:pending` (отметка последней отправки, `markImportPending`),
а VPS по третьему таймеру `court-import-poll.timer` (будни 08:00–20:55 каждые 5 мин,
`Persistent=false`) гоняет [ops/vps-run/import_poll.sh](../../../ops/vps-run/import_poll.sh): по территории
ОДИН дешёвый `GET /import-pending` (`handleImportPendingGet`, KV get — не list; авторизация
`importChannelAuthOk`, на 401 фолбэк на владельческий секрет), при новой отметке — сразу
`import_dumps.sh <клон> --anywhere`, отметка в `.runtime/import_pending_seen` пишется ПОСЛЕ запуска при
любом коде (повтор провалов — работа слотов, иначе поллер крутил бы одну запись каждые 5 минут);
занятый `.run.lock` (утренний парсинг/слот) = пропуск тика БЕЗ отметки — у `import_dumps.sh` «занято»
и «очередь пуста» выходят одним кодом 0, поэтому лок поллер проверяет сам. Слоты 12–20 и утренние
`run_imports` остаются страховкой. Админка в режиме vps снова ждёт итог, но поллит РЕЖЕ GitHub-пути:
`IMP_VPS_POLL_TICK_MS`=60 с, потолок `IMP_VPS_POLL_GIVEUP_MS`=12 мин (≈12 list'ов на отправку против
10 у GitHub), на потолке — «сервер пока не взял… повтор в слот ЧЧ:ММ» и форма чистится (тело в KV);
`impPollResult`/`acPollResult` параметризованы `opts` {tick, giveup, giveupHtml, clearForm}. Стражи —
`test_poll_timer_is_frequent_and_not_persistent`, `test_poller_checks_lock_before_run_and_marks_after`
(test_vps_run.py), `test_pending_*`, `test_admin_polls_slowly_under_vps` (test_mac_import_dumps.py).
TTL тела дампа/задания 24 ч → **72 ч** (`IMPORT_DUMP_TTL` ↔ `DUMP_TTL`): пятничная вставка после
20:00 иначе не доживала до понедельника. Маркер исполнителя — `CM_IMPORT_SOURCE=vps` из
`vps_env.sh` (`SRC`/`SRC_LABEL` в import_dumps.sh: отчёт `source:"vps"`, коммиты «(VPS)», тексты
ошибок «VPS: …»; без переменной — прежний «mac»), подписи админки — «🖥 обработано сервером»,
«повторит сервер в следующий слот»; старые записи GitHub/Mac рендерятся прежними текстами.
GitHub-путь остаётся аварийным: `IMPORT_EXECUTOR = "github"` + deploy (действует на НОВЫЕ записи;
стоящие в `queued` заберёт Mac-пульт `import_all.sh --anywhere`). ⚠️ Пометки «лист не нужен»
(`writ_waiver`) флага не знают — всегда GitHub, очередь их не берёт. ⚠️ Форк: сначала merge
(старый jq форка `queued`-пачки не выберет никогда), потом deploy Worker'а с флагом. Стражи —
`TestImportExecutorWiring` (test_mac_import_dumps.py). Абзац ниже — история канала дочитки, его
механика (эндпоинты, jq-правила, грейсы GitHub-пути) жива и описывает VPS.

⚠️ **Операторские импорты резерв тоже подхватывает (16.08.2026; с 23.08.2026 — оба канала)** —
...
суток = TTL KV, и облако его либо не довело, либо довело с
`fetch_fail`/`card_failed`; запись «идёт» моложе 15 мин не трогаем —
...
заход запись не выберет. **С 23.08.2026 очередь разбирает и ТОЧЕЧНЫЕ ПАЧКИ**
(`import:case:`, `GET /add-case-job?key=` → `scripts/add_cases_targeted.py` →
`ops/add_case_result_body.jq`): у пачек своя цена блока — в ССЫЛОЧНОМ режиме
(единственный путь для капчёвых судов) роль банка решается только по карточке,
и без неё строка выбрасывается ЦЕЛИКОМ, card-blind записи не выходит; пачка
тихо истекала в KV через сутки. Четыре блокера снимались одним заходом:
(1) `ST_FETCH_ERROR` не входил в `_COUNTER_STATUSES` и сливался в «отказано» —
машинного признака потери не было ни в отчёте, ни в KV (теперь свой счётчик
`fetch_error`, проводка jq → whitelist → `acResultText`, страж
`test_targeted_counters_reach_operator`); (2) очередь выкидывала `kind:"case"`
строкой `select`; (3) искала `fetch_fail`/`card_failed`, которых этот канал не
шлёт; (4) `GET /add-case-job` принимал ТОЛЬКО `PUSH_SECRET`, которого на Mac
нет физически — роут переведён на общий `importChannelAuthOk` (как
`/import-dump` 16.08.2026). ⚠️ **Грейс живого облачного джоба у пачек СВОЙ** —
`CASE_STARTED_GRACE`=3000 с против дамповых 900: `add_cases.yml` стоит на
`timeout-minutes: 45`, и дамповые 15 мин пустили бы Mac писать в те же файлы
посреди живого джоба; статус `dispatched` под тем же грейсом — отдельная ветка
(у GitHub-группы `cases-data-write` живёт ОДИН pending, запись может простоять
в нём всю очередь). ⚠️ **Пачка повторяется ЦЕЛИКОМ**, а не одними упавшими
строками: отчёт перезаписывает счётчики записи журнала, и повтор двух строк из
двадцати оставил бы оператору «+2 добавлено», спрятав исходные 18. ⚠️ Ранний
выход «капчёвых судов нет — выходим» СНЯТ: пачки есть на любой территории, и
он оставил бы ХМАО вовсе без дочитки; тишину держит отсутствие
`worker.<регион>`. Пометки «лист не нужен» (`import:writ:`) очередь не берёт
никогда — к судам они не ходят. **Кто отработал запись, видно оператору**
(`source` ∈ {github, mac}, белый список значений в Worker'е): строка журнала
получает «🖥 повтор с локальной машины · время» — без маркера обещание сводки
«повторит локальная машина» было бы непроверяемым. Три вещи держать общими с
облаком, иначе разъедутся молча (этим проект болел дважды): пейлоады отчёта —
[ops/import_result_body.jq](../../../ops/import_result_body.jq) и
[ops/add_case_result_body.jq](../../../ops/add_case_result_body.jq), список коммитимых
...
- **Листа не будет ВРУЧНУЮ** (`writ_waived`; с 25.08.2026 это именно
  ЗАКРЫТИЕ): иск удовлетворён, но лист не понадобится, например ответчик
  погасил долг добровольно. Из карточки суда это НЕ видно — проверка 16 287
  событий дала ноль вхождений «погаш»/«добровольн»/«исполнительн», поэтому
  канал ручной. В карточке «Закрыть дело — ИЛ не нужен» обе роли выбирают суд
  и вводят номер; Worker принимает точную тройку (домен, `srv_num`, номер),
  чтобы не перепутать площадки одного сайта, и передаёт job через
  `POST /admin/writ-waiver` → KV `import:writ:<uuid>` →
  [mark_writ.yml](../../../.github/workflows/mark_writ.yml) →
  [scripts/mark_writ_waived.py](../../../scripts/mark_writ_waived.py). Авторитетный
  Python-гейт разрешает закрыть только дело трека со статусом «Решено», без
  enforcement-листа и без жалобы. Запись получает
  `first_instance.writ_waived` = {reason, at, by} и СРАЗУ переносится из
  cases_bank.json в cases_bank_archive.json; снятие пометки возвращает её в
  активные, если обычные сроки архивации ещё не истекли. Причины закрытым
  списком: `debt_paid`/`not_requested`/`other` (свободного текста нет —
  `first_instance` публичен). Ключ отдельный от пересчитываемого
  `writ_expected`; `_is_bank_track_archived` считает ручную пометку немедленным
  архивным основанием, но жалоба/заявление об отмене имеют приоритет. Гейт
  `bank_writ_awaited` по-прежнему выключает напоминания до раскладки.
  Админка НЕ печатает всю очередь (на Урале 910 строк): основной путь — форма,
  ниже только строки с `court_archived_at` (`bank_court_archived_date`), где
  суд сам передал дело в архив, а ИЛ нет (на данных 25.08.2026 — 3 подсказки).
  Кейс, с которого началось: уральское 2-28/2026. Фронт читает пометку через
  `writWaivedInfo`; тесты — [scripts/tests/test_writ_waiver.py](../../../scripts/tests/test_writ_waiver.py).
- **Листа не будет** (`bank_writ_expected`, с 31.07.2026 — находки юриста в
  ...
  удовлетворением лист получают — их не трогаем. **(3) Оставлено без
  рассмотрения (ст. 222 ГПК, с 03.09.2026, решение юриста)** — тоже листа
  не будет: `fi_left_unconsidered` читает ТОЛЬКО поле «Результат» (как
  merged: в истории движения лежит определение прошлого круга, отменённое
  по ст. 223 ч. 3), а карточка статус НЕ флипает («В производстве» у всех
  9 дел ХМАО) — поэтому ветка «листа не будет» в `_is_bank_track_archived`
  принимает и этот статус, иначе дело застряло бы в активных навсегда.
...
- **Дайджест**: секция «🏦 ИСКИ БАНКА (N)» — компакт, одна строка на дело,
  ...
  **Сокращение дайджеста 21.08.2026** (юрист: «очень длинный дайджест по ЕКБ,
  даже не помещается в мобильной версии»; замер того дня — секция банка 82%
  выпуска Урала и 75% ХМАО, то есть проблема ОБЩАЯ, Урал лишь показал масштаб).
  Пять правок, все в общем коде. (1) **Пересказ «Почему» — только при исходе
  ПРОТИВ банка**: `bank_act_why_eligible` перешёл с чёрного списка на белый
  (`BANK_ACT_WHY_OUTCOMES` = «против банка» / «частично против банка») — прежний
  отсекал лишь полное «в пользу банка» и пропускал частичные победы, которые
  юрист и принёс как пример (2-3034/2026, 2-3689/2026, 2-4131/2026, 2-3996/2026
  Кировского р/с ЕКБ, все «удовлетворено частично»). Строка «📄 текст решения
  опубликован» остаётся — решение юриста. (2) **Процессуальные закрытия не
  пересказываются** (`act_why_worth_it`, `ACT_WHY_SKIP_VERDICTS`: прекращено /
  оставлено без рассмотрения / возвращено) — гейт общий для банк-секции и
  основной 3.6. ⚠️ Отдельного гейта мало: `bank_side_outcome_fi` отдаёт для
  закрытий «против банка» ПО РОЛИ, и белый список исходов их бы пропустил.
  ⚠️ Тем же заходом 3.6 научилась печатать ПРИЧИНУ закрытия (`_FI_CLOSURE_HEADS`
  + `fi_closure_reason`, как в 3.5): секции взаимоисключающие, дело с
  опубликованным текстом уходит ТОЛЬКО в 3.6, где стояло голое «Итог:
  прекращено» — без причины правка отняла бы объяснение целиком. (3) **Свёртка
  «вступило в силу»** (с 21.08.2026 общее тело `_split_bank_fold` обслуживает и
  «⚠️ ИЛ не выдан» — `split_bank_overdue_fold` / `_bank_overdue_fold_line`,
  список по УБЫВАНИЮ срока: это рабочая очередь; ⚠️ `_insert_fold` считает
  позицию ПОСЛЕ уже вставленных свёрток той же группы — иначе каждая следующая
  вытесняла бы предыдущую наверх, и порядок вывода стал бы обратным порядку
  вызовов) (`split_bank_force_fold` / `_bank_force_fold_line`, порог
  `BANK_FORCE_DIGEST_FOLD`): самый массовый рутинный класс на обеих территориях
  (14 строк из 80 у Урала, 8 из 26 у ХМАО), событие календарное. В отличие от
  intake-свёртки номера дел ОСТАЮТСЯ в HTML — строка перечисляет их ссылками
  (юрист просил «раскрывающийся счётчик», список прямо в строке отвечает лучше
  фронтового костыля: `sanitizeDigestHtml` всё равно срезал бы атрибуты, а
  Telegram — тег `<details>`). Поэтому линтер и `llm._collect_case_numbers`
  трогать не пришлось; задет только `_check_section_counters` — он прибавлял 1
  за строку, теперь считает ОБЁРНУТЫЕ номера с полом в единицу. ⚠️ Считать все
  номера подряд нельзя: строка новой апелляции несёт номер 1-й инстанции хвостом
  голым и дала бы двойной счёт (страж `test_appeal_tail_number_not_double_counted`).
  (4) **Сокращалка сторон** — `_MTU_HEAD` знал только ХМАО-форму, а уральские
  «Территориальное управление Федерального агентства…» (114 симв., 13 вхождений)
  и «ТУ Росимущества по…» не сокращались вовсе; замена теперь функцией
  `_rosim_short` («МТУ Росимущество» / «ТУ Росимущество» — это РАЗНЫЕ органы).
  ⚠️ `\b` перед `МТУ`/`ТУ` load-bearing: без границы слова «МТУ …» совпало бы с
  офсета 1 как «ТУ …», оставив осиротевшую «М». Там же `_shorten_heritage`
  чистит пустые скобки ДО разбора ФИО (одна строка чинит оба симптома: «насл.
  имущество Клешнина Маргарита Александровна ( )» → «насл. имущество Клешнина
  М.А.») и матчит склеенную ячейку «наследственноеимущество» ленивым `\w*?`.
  Плюс потолок перечня `cap_parties` (первые 2 стороны + «и ещё N») — ТОЛЬКО в
  компакт-строке банк-секции: результат `shorten_party_name` уходит в case_meta
  LLM-пересказчика, где «и ещё 3» испортило бы пересказ. (5) **Гард латиницы**
  (`summary_language_ok`, llm.py): выпуск 21.08 разослал «послужили Creditный
  договор» и «доводы о lack of доказательств» — прежний гард требовал лишь ≥40%
  кириллицы. ⚠️ Проверка стоит и на ЧТЕНИИ кэша: кэш-хит идёт до всех чисток, и
  испорченный пересказ жил бы в `.act_summaries.json` вечно (версию «v3-detailed»
  при этом НЕ бампаем — бамп заново оплатил бы все хорошие пересказы).
  Итог замера: ХМАО 5 754 → 4 880 видимых символов (−15%), Урал 19 285 → 13 866
  (−28%), плюс −1 536 на Урале от снятых пересказов. **Явно отклонено юристом**
  (показаны образцы на боевых данных): свёртка решений «⚖️ иск удовлетворён» и
  свёртка заседаний по искам банка — печатать подробно.
  ...
  **Эскалация напоминаний об ИЛ (21.08.2026, вопрос юриста «как часто это
  шлётся?»).** Оказалось — РОВНО ОДИН раз за жизнь дела: гейт стоял по
  значению est, и `fi_writ_overdue` объявлялся на 30-й день, после чего дело
  молчало до архива на 180-й. Проверка на данных: за всю историю обеих
  территорий 12 объявлений на 12 РАЗНЫХ дел, смены маркера — ноль, а на
  21.08.2026 молча зависли 50 дел (ХМАО 16, Урал 34), рекорд — 171 день без
  листа. Теперь маркер `writ_overdue_emitted` СОСТАВНОЙ — «est|достигнутый
  порог», а пороги даёт `config.writ_overdue_steps()`: от
  `BANK_WRIT_OVERDUE_ALERT_DAYS` с шагом `BANK_WRIT_OVERDUE_REPEAT_DAYS`=30 до
  архивного потолка, то есть 30/60/90/120/150 (`0` возвращает прежнее «один
  раз»). ⚠️ Миграции НЕТ намеренно: старое значение без «|» читается как
  «первый порог объявлен» — ровно то, что и произошло. ⚠️ Эпоха
  `BANK_CALENDAR_EVENTS_SINCE` гасит теперь ТОЛЬКО первый порог: она защищала
  от паводка при вводе фичи, а на повторных порогах похоронила бы ровно те
  зависшие дела, ради которых эскалацию и завели (у 2-28/2026 Урала со
  171 днём следующий порог 180 совпадает с архивацией). Сдвиг est по-прежнему
  обнуляет лестницу. Всплеск гасит свёртка: симуляция на боевых данных дала
  до 8 просрочек за день у ХМАО и до 19 у Урала (решения вступают в силу
  пачками), а со свёрткой в дайджест идёт максимум 3 строки в день. Стражи —
  `TestBankCalendarEvents` (пороги, legacy-маркер, эпоха, REPEAT=0),
  `BankOverdueFoldTest`, `LintBankOverdueFoldTest`.
  ...
  **С 13.08.2026 — LLM-пересказ «Почему»** у `fi_act_text_published` с
  ...
  фронта. **С 21.08.2026 тело этого вызова — общий хелпер
  `_attach_bank_act_analyses`, и зовут его ДВА места: `main_json` и
  `main_replay_last`.** Причина — Mac-режим: на машине юриста ключей LLM
  нет, разбор считает только replay, а он чинил ОДНУ картотеку (за тем и
  коммитит `cases.json`, см. `replay_on_push.yml`) — банк-трек оставался с
  `source="raw_act"`, то есть с СЫРЫМ текстом мотивировки под заголовком
  «AI анализ» навсегда (5 записей Урала за 20–21.08, вычищены разово).
  Поэтому же в коммит replay добавлены `cases_bank.json` +
  `cases_bank_events.json`. ⚠️ Replay обязан грузить трек через
  `load_bank_json` и отдавать `save_bank_json` ВЕСЬ загруженный dict:
  events-файл перезаписывается целиком из переданных записей, а
  `version`/`track`/`archived_count` переживают только копированием из того
  же dict. ⚠️ Второй предохранитель — гард «нет ключа LLM» перед ОБЕИМИ
  врезками attach в `main_json`: без него raw_act-фолбэк, задуманный под
  редкий 429, на машине без ключей срабатывает каждое утро. Стражи —
  [scripts/tests/test_act_analysis_guards.py](../../../scripts/tests/test_act_analysis_guards.py).
...
- **Ожидание ИЛ** (`legal_force_est`): `split_bank_track` штампует в
  ...
  `BANK_WRIT_WAIT_MAX_DAYS`. ⚠️ **Расчёт только у РЕШЁННОГО дела (03.09.2026):**
  `bank_legal_force_est` возвращает None, если нет `decision_date` и статус
  ≠ «Решено» — фолбэк на `hearing_date` остаётся лишь у решённых записей без
  штампа (архив/легаси). До гейта hearing_date живого дела (= БУДУЩЕЕ
  заседание) давала «вступление в силу» от ещё не состоявшегося заседания:
  штамп стоял у 342 из 554 активных исков ХМАО, drawer печатал «Вступило в
  силу … (расч.)» у дел без решения; `writ_awaited_since` шёл следом. Фронт
  держит второй рубеж (`сила&&kdResultPresent`) — снимок данных до ближайшего
  прогона штамп ещё несёт. Миграции нет: оба ключа пересчитываются каждым
  прогоном.
- **«Ключевые даты» drawer'а (03.09.2026, разбор юриста):** три хелпера в
  app.js рядом с `HEARING_DATE_LABELS` — `stageResolvedDate` (дата под
  строкой решения: fi — замороженная `decision_date` → `hearing_date`
  решённого/прошедшая → `event_date`; ap — заседание рассмотрения; cs —
  `decision_date`), `resolvedRowLabel` («Решение» / «Рассмотрено» /
  «Определение» — для возврата, прекращения, снятия, без рассмотрения и
  присоединения) и `hearingRowState` (строка заседания). До этого под
  «Решение» печаталась `event_date` — дата ПОСЛЕДНЕЙ строки движения любого
  рода («Дело сдано в отдел», «Изготовлено мотивированное решение»): 48 из
  52 решённых дел основной картотеки, 120 из 188 у трека, 85 из 91 на
  вкладке апелляции («Передано в экспедицию»); настоящая дата решения при
  этом стояла строкой выше как «Последнее заседание». Решение юриста: у
  решённого дела при совпадении дат — ОДНА строка «Решение», «Последнее
  заседание» остаётся только у заседания ПОСЛЕ решения (расходы,
  индексация); у нерешённого с ПРОШЕДШЕЙ датой — «Последнее заседание …
  (прошло/вчера, следующее не назначено)» вместо «Заседание». Тем же заходом
  эмит `fi_resolved` и бэкфилл `migrate_stages` морозят `decision_date` из
  события «Вынесено (заочное) решение» (`fi_decision_date_from_events`),
  фолбэк — прежняя `hearing_date`. Тем же днём фронт узнал исход
  **`transferred`** («Передано по подсудности, подведомственности» — поле
  карточки, «Дело передано на рассмотрение другого суда» — колонка события):
  раньше `normalizeResult` отдавал 'pending', пилюля исхода не рисовалась, а
  drawer терял строку решения (2-716/2026). Ключ проведён по всем словарям
  (`FI_RESULT_LABELS`/`RESULT_LABELS` «Передано по подсудности»,
  `RESULT_ICONS` «→», `FI_RULING_RESULTS`, `fiProceduralEnding`), favor —
  нейтральный при любой роли (дело продолжится в другом суде). Стражи —
  [scripts/tests/test_frontend_drawer_dates.py](../../../scripts/tests/test_frontend_drawer_dates.py),
  `TestLegalForceOnlyWhenDecided`/`TestLeftUnconsidered`/
  `TestDecisionDateFrozenFromEvent` в test_bank_track.py.
...
- `LLM_SUMMARY_PROVIDER_FALLBACK` — фолбэк-провайдер пересказов актов (дефолт 1): бесплатный пул OpenRouter лёг целиком → одна попытка боевого Claude haiku при живом `ANTHROPIC_API_KEY`; `0` — остаться чисто на бесплатном пуле (инцидент 28.08.2026: оба акта Урала ушли сырым отрывком навсегда).
...
- `BANK_FORCE_DIGEST_FOLD` — порог свёртки «✅ решение вступило в силу — ожидаем ИЛ» в дайджесте (3; `0` — печатать подельно). Прокинут во все три workflow.
- `BANK_WRIT_OVERDUE_REPEAT_DAYS` — шаг эскалации напоминания «ИЛ не выдан» (30 → пороги 30/60/90/120/150; `0` — прежнее «один раз за жизнь дела»). Прокинут ТОЛЬКО в update_cases.yml: календарный проход живёт в `main_json`, replay событий не эмитит.
- `DIGEST_PARTIES_MAX_LEN` / `DIGEST_PARTIES_KEEP` — потолок перечня сторон в компакт-строке банк-секции (60 символов / 2 стороны, дальше «и ещё N»; `0` — не резать).
...
- `FETCH_TIMEOUT_CONNECT` / `FETCH_TIMEOUT_READ` — таймаут запроса к суду, РАЗДЕЛЬНЫЙ (10 и 65 с). Соединение встаёт за 0,05 с, а ответ портал готовит десятки секунд: замер 24.08.2026 дал 26–58 с на `sud_delo` при 0,2 с на корень сайта, и прежние общие 30 с рубили верхнюю половину распределения. Потолок 65 задаёт сам сервер — проба висела 60,26 с, после чего портал разорвал соединение без ответа. На здоровом дне значение невидимо: таймаут — потолок, а не задержка.
- `CM_LOG_KEEP_DAYS` — сколько дней хранить ротированные логи резерва (14; `parse_and_push.log`/`import_dumps.log` уносятся в `<имя>-ГГГГММДД.log` при первом запуске нового дня).
...
- **Сводка** — `GET /admin/visits` (`handleAdminVisits`, [worker.js:2732](../../../cloudflare-worker/worker.js:2732)), **только владельцу**: читает историю ОДНИМ `list` по префиксу и **без единого `get`** (`metadata` приходит вместе с ключами), пагинация по курсору ОБЯЗАТЕЛЬНА (бескурсорный `handleAdminImportLog` копировать нельзя — при >1000 ключей молча теряется хвост). Наружу уходит только огрызок `vid` (6 символов). ⚠️ Поллинга НЕТ и оператору не отдаётся: lists на free-tier тоже 1000/день, а операторов на Урале ~14.
- **UI**: плитка «Посещения» в пульте (owner-only) + карточка в секции «Подписчики». ⚠️ Карточка стоит ВНЕ `#root` — тот перерисовывается `render()`'ом целиком на каждое нажатие в поиске по подписчикам. ⚠️ Спарклайн — СВОЙ класс `.visits-spark`, а НЕ `.health-spark`: у последнего в мобильной выборке `display:none`, а админку юрист смотрит с телефона. Число колонок `.pult` обязано совпадать с числом плиток владельца (5; с «Импортами» — 6 в два ряда по три).
- **Чего счётчик не покажет** (сказано и в самой карточке): один человек с телефона и ноутбука = два «посетителя»; очистка данных сайта, инкогнито или другой браузер = новый; один компьютер на двоих = один. Ретроспективы нет — счёт с 31.08.2026. Он меряет заходы на дашборд, а не пользу инструмента: часть коллег читает только Telegram-дайджест и на сайт может не заходить вовсе.
- Стражи — [scripts/tests/test_visit_counter.py](../../../scripts/tests/test_visit_counter.py) (27 тестов: потолок writes, анонимность тела, неймспейс `lsKey`, отсутствие IP/UA, выключатель, list-бюджет, вёрстка пульта и карточки).

## Админка подписчиков

URL: `https://api-hmao.delosud.ru/admin?secret=<OWNER_SECRET>` (с 27.08.2026 основной адрес Worker'а — свой домен: часть операторов связи режет `*.workers.dev` по имени; прежний `https://court-monitor-trigger.7selivanov-a.workers.dev/admin?…` жив как фолбэк, фронт перебирает адреса через workerFetch, стражи — test_worker_fallback.py; у зоны delosud.ru ВЫКЛЮЧЕН ECH — не включать, ручка только в API `/zones/<id>/settings/ech`). Открывается в браузере (мобильно тоже). HTML-страница вынесена в [cloudflare-worker/admin_page.js](../../../cloudflare-worker/admin_page.js) (`renderAdminHtml(secret, role, cfg)`, wrangler бандлит импорт сам); серверные эндпоинты — в [cloudflare-worker/worker.js](../../../cloudflare-worker/worker.js). ⚠️ Вся страница — один template literal: внутренний JS пишется без backtick'ов и `${`, backslash удваивается. Naive-таймстампы из data/*.json (Python на UTC-раннере пишет без «Z») страница парсит как UTC (`parseIso`).

**Роли (с 16.07.2026):** owner (`OWNER_SECRET`) — всё; operator (`OPERATOR_SECRET`, один общий на сопровождающих капчёвых судов; не задан — роль неактивна, у ХМАО так: дампы капчёвого Суда ХМАО-Югры с 04.09.2026 вставляет сам юрист под owner) — статус+здоровье+живой лог+секция «Импорт дел». Гейт — `resolveAdminRole`/`requireAdminRole` (worker.js): чужой секрет → 401, оператор на owner-эндпоинте → 403 (реальный запрет на сервере, скрытие в UI — `data-owner-only` + `html[data-role]`). `DISPATCH_WORKFLOWS` — `{inputs, roles}`: update_cases/test_digest — owner, import_cases/add_cases — обе роли. Секция «Импорт дел» (`#import`): **с 10.08.2026 вкладка видна ОБЕИМ ролям на ЛЮБОЙ территории** — первой карточкой блок «Добавить дела» (точечное добавление до 20 строк: номер/ссылка, построчная валидация против `region.*` cases.json, POST `/admin/add-case`, поллинг того же журнала тиком 60 с с потолком 40 мин — пачка номеров идёт дольше дампа, а каждый тик стоит KV-list; имя оператора общее с дамповой формой через `admin_operator_name`); дамповая часть ниже — dropdown gated-судов из `region.fi_courts` (нет ни одного gated-суда — прячется только она + светофор, `.imp-grid` схлопывается в одну колонку, журнал грузится `logonly=1`; у ХМАО так было до 04.09.2026 — теперь там закреплённая апелляция «Суд ХМАО-Югры — апелляция»), вставка rich-paste/файл, поллинг журнала (`/admin/import-log`), история импортов (записи `kind:"case"` рендерятся «📌 точечно · N стр.»), **светофор свежести по судам** (когда каждый суд импортировался в последний раз: зелёный ≤7 дн, жёлтый ≤14, красный дольше/ни разу; данные — вечные ключи `import:last:<домен>`, пишутся на done в `/import-result`). **С 17.07.2026 — операторский UX (394cd14):** оператору секция открыта по умолчанию (с 02.08.2026 — вкладка по умолчанию, `TAB_DEFAULT`; серверная перестановка `IMPORT_SECTION` убрана), светофор раскрыт и кликабелен (клик по суду = выбрать его в форме, слушатели — делегированием), 5-я плитка пульта «Импорты» (N просрочено; обеим ролям, без gated-судов скрыта — у ХМАО видна с 04.09.2026, `.pult.has-import` в мобильном медиа-блоке обязателен), drag-n-drop файла в поле вставки + индикатор «что уйдёт» (файл побеждает вставку) + предпроверка «есть ли ссылки» до отправки, статус ожидания с таймером, сбой cases.json → алерт с «Повторить» (owner'у — тихо, как раньше). **Автоопределение суда по вставке (17.07.2026):** хост из абсолютных ссылок карточек подставляет суд в dropdown сам (пока оператор не выбирал вручную — флаг `impCourtTouched`), конфликт — заметка «⚠ ссылки ведут в …» с кнопкой «выбрать этот суд», отправка чужого дампа блокируется на клиенте, Worker'е (400) и в импортёре (`EXIT_WRONG_COURT`). ВСЕ URL данных страницы выводятся из `CASES_DATA_URL` Worker'а (`adminPageConfig()`) — хардкод сломал бы админку территорий. **Операторский путь дочищен 02.08.2026:** после успешного импорта форма ОЧИЩАЕТСЯ (поле + файл + `impRunDetect`) и светофор перерисовывается из кэша `impLastFreshMap` без похода в KV — раньше оператор шёл очередью судов, и следующий Ctrl+V клеился в конец прошлого дампа («⚠ ссылки нескольких судов», отправка заблокирована, кнопки «очистить» нет); при `failed` вставка сохраняется для повтора. Ссылка на суд несёт `srv_num` выбранной площадки (+`delo_id=1540005&name_op=sf`) — голая вела на первую площадку домена, а часть судов заведена ТОЛЬКО как `srv_num=2` (серверные предохранители этого не ловят: хост и delo_id совпадают). Первый рендер светофора подставляет самый просроченный суд (`impFreshAutoPicked`; `impCourtTouched` при этом НЕ ставится — автоопределение по вставке должно сохранить право переключить). Индикатор и предпроверка считают `name_op=case` («дел на странице: N»), а не все `a[href]` — прежний счётчик показывал 137 при десятке дел, а тест на голый `<a>` пропускал вставку без единого дела. Светофор показывает «+7 из 24» (`rows` в вечном ключе `import:last:*`, с 02.08.2026 — у старых записей его нет, фолбэк «+7»). **С 14.08.2026 в списке видны ПОСТОЯННЫЕ СУДЕБНЫЕ ПРИСУТСТВИЯ**: прежний дедуп `impCourts` по домену выкидывал вторую площадку сайта (Пышма у Камышловского, Ачит у Красноуфимского — в реестре с 16.07.2026), и её дела не импортировал никто. Ключ строки — `impCourtKey` = «домен|srv_num» (`impDomainOf` отрезает домен там, где нужен он: POST `/admin/import-dump`, сверка с хостами дампа, автоопределение — у площадок одного суда хост ОБЩИЙ, сравнивать надо домены, иначе выбранное присутствие молча перевыбирается на первую площадку). Свежесть остаётся по домену (вечный ключ `import:last:*` серверный, площадок не различает) — у суда и присутствия дата общая, что для регламента честно: оператор берёт обе выдачи за один заход. Стражи — [test_admin_pult.py](../../../scripts/tests/test_admin_pult.py). Мобильная форма — своим медиа-блоком ПОСЛЕ правил `.imp-row` (общий блок «Мобильная раскладка» стоит в файле раньше и при равной специфичности проигрывал), поля 16px — iOS зумит всё мельче. **С 02.08.2026 вкладка на ≥1200px в две колонки** (`.imp-grid`: форма слева, свежесть+история справа) — иначе на 1440px операторская была одной узкой колонкой и рабочая очередь не читалась одновременно с формой; обеим колонкам `min-width:0` (зона вставки с таблицей суда распирала бы трек), ≤1200 — обратно в одну.

**Разбор операторской (23.08.2026).** Роль живая только на Урале (у ХМАО `OPERATOR_SECRET` не задан, gated-судов ноль), секрет ОДИН на всех сопровождающих, капчёвых судов у них 56 из 69. Семь правок, четыре из них — починка. **(1) Пометки «лист не нужен» в журнале рисовались дамповой веткой**: `impResultText` знал только `kind:"case"`, и запись `writ_waiver` печаталась как «готово ? · Имя · +0 в картотеку» — счётчики `waived`/`updated`/`cleared` доезжали до KV полностью, рвалось ТОЛЬКО третье звено. Теперь `wwResultText` + подпись канала «🚫 лист не нужен · N дел» рядом с «📌 точечно · N стр.» (суда у пультовых операций нет вовсе, и `impCourtNameByDomain[…] || "?"` давал «?»). **(2) Построчный отчёт пометок был «[object Object]»**: `mark_writ_waived.py` копил в `lines` словари (оба других канала — строки), а Worker кладёт их через `body.lines.map(String)`; теперь общий `report_line` с маркерами в стиле остальных каналов (`[WAIVED]`/`[UPDATED]`/`[CLEARED]`/`[NOT FOUND]`/`[REFUSED]`). **(3) Светофор обходил СОБСТВЕННУЮ защиту.** Серверный гейт `cardsUnread === 0` (worker.js, заведён после Верх-Исетского 16.08.2026) не пишет `import:last`, если карточки не читались, — но клиентский фолбэк по журналу этого условия не имел, и запись со `status:"done"` (она новее карты) перекрывала её: суд, у которого дамп потерял 12 исков, красился зелёным «импортирован сегодня». Зеркало гейта добавлено в `renderImportFreshness`, страж — `test_freshness_client_mirrors_server_gate`. **(4) Склонения**: `wwPlural` переименован в общий `plural`/`nPlural` и проведён через счётные фразы обеих сводок («1 карточек дочитано» → «1 карточка дочитана»). ⚠️ Телеграфные ярлыки корзин («2 ИЛ уже выдан», «1 отработавших (иски банка)») НЕ трогали — их держат стражи `test_bank_counters_reach_operator` буквальными подстроками. **(5) Здоровье парсеров названо честно**: `parse_health.json` наполняется только по `courts_for_search`, а тот ИСКЛЮЧАЕТ `search_gated` — карточка описывала 13 судов из 69, ровно те, которых оператор не ведёт, и «все N ok» читалось как «мои суды в порядке». Охват подписан в `health-updated` (обе роли), а плитка «Парсеры» у оператора заменена на **«Карточки судов»**: для капчёвого суда чтение карточек — весь канал мониторинга, а видно оно было только внутри отчёта одного импорта. Считается `collectCardTrouble` по УЖЕ загруженному журналу (ноль новых запросов), полная причина — в `title` плитки (подпись клампится тремя строками: `card_fail_reason` — свободная фраза, и на телефоне она растягивала весь ряд пульта). Та же находка даёт пометку «⚠ <когда> карточки не читались» в строке светофора — без неё красный суд, импортированный сегодня, необъясним. ⚠️ Пометка стоит ОТДЕЛЬНЫМ элементом со своей строкой (`flex-basis:100%`): мета говорит про последний ЗАСЧИТАННЫЙ импорт, пометка — про последнюю ПОПЫТКУ, это разные дни, а склейка в одну строку ещё и съедала имя суда многоточием (`.health-name` схлопывается раньше, чем сработает перенос). **Доводка на боевом реестре (23.08.2026, тот же день):** проверка шла на выдуманных 14 судах, а на настоящем Урале их 54 — и вылезло то, чего короткий список показать не мог. Список получил потолок `FRESH_VISIBLE`=12 + свёртку «Остальные N судов» (`freshList`, зеркало карточки «Здоровье парсеров» с её `VISIBLE`=8): без него вкладка на телефоне вырастала до 6,3 экрана, из них 4,3 — один список, и оператор пролистывал всю территорию до истории импортов (стало 3,1; на десктопе 2479→1102px). Потолок применён к ОБЕИМ веткам рендера — и «все суды», и «мои суды»; режим правки набора не капается намеренно (там ищут свой суд глазами по алфавиту). Пометка про карточки перекрашена в `--fg-3`: на боевых данных её носят 10 строк из 54, до четырёх подряд, и красным она превращала рабочую очередь в стену алярма — тревогу и так поднимает плитка «N судов не отдали». Формулировка стала «⚠ попытка <когда>: карточки не читались»: слово «попытка» обязательно, иначе у суда без единого успешного импорта две строки читались как противоречие («ни разу не импортировался» + «16 дн назад карточки не читались»), а на абсолютной дате `relTime` фраза разваливалась вовсе («23.07.2026 карточки не читались»). Что проверку ПЕРЕЖИЛО без правок: имена до 48 символов не обрезаются, `<select>` на 54 пункта, пары площадок различимы по имени, сводка со всеми 17 корзинами укладывается в три строки, история на 20 записей и отчёт на 100 строк. **(6) «Мои суды»** (`admin_my_courts` в localStorage; админка у каждой территории на своём origin, неймспейс не нужен — так же плоско лежит `admin_theme`): до этого каждый из ~14 операторов видел общую очередь на 56 судов, а автоподстановка выбирала самый просроченный из ВСЕХ — почти наверняка чужой. Свои суды идут наверху рабочей очередью, остальные — в свёртке «Прочие суды»; бейджи и плитка «Импорты» считают по подсети (`counted`), автоподстановка — из неё же. ⚠️ ПУСТОЙ набор = прежнее поведение (все суды), до первого выбора не меняется ничего. ⚠️ Кнопки управления набором — в теле свёртки (`#imp-my-bar`), НЕ в `<summary>`: кнопка внутри summary переключала бы саму свёртку (та же грабля, что в карточках подписчиков). ⚠️ Выход из режима правки сбрасывает `impFreshAutoPicked` — иначе форма осталась бы на чужом суде, выбранном первым рендером; `impCourtTouched` при этом не трогаем (ручной выбор сильнее). **(7) Порядок и сводка.** Карточки вкладки склеиваются по роли (оператору `DUMP_CARD + AC_CARD`, владельцу наоборот): у оператора еженедельные дампы были ПОД эпизодическим точечным добавлением, и светофор начинался ниже первого экрана; у владельца дампов нет вовсе, ему порядок прежний. Точечное добавление у оператора — свёрткой (`.ac-fold`), инструкция из 6 шагов — свёрткой, раскрытой пока пуст `admin_imp_steps_seen` (ставится на первом `done`). Имя оператора — ОДНО поле `#imp-name` в шапке секции; ⚠️ обязано жить ВНЕ `.imp-form`, иначе на территории без капчёвых судов пряталось бы вместе с ней (`acSend` и `wwSend` читают его же). Сводка результата разложена на `impResultParts` → `{parts, problems, skipped}` + `impVerdict`: живой статус после отправки рендерит `impResultHtml` (вердикт «Готово: заведено N дел» / «Ничего не заведено — карточки не открылись; повтор подхватит локальная машина», ниже «Проблемы», «Заведено», «Пропущено»), история — прежней однострочной склейкой `impResultText` в ТОМ ЖЕ порядке корзин. **Формулировки провалов пересобраны 23.08.2026** (наблюдение юриста): «повторите дамп, когда суд отвечает» писалось 16.08, ДО появления очереди резерва, и звало оператора делать работу, которая сделается сама — очередь берёт запись по `fetch_fail`/`card_failed` и переделывает её в тот же день. Теперь сводка обещает локальную машину (`impRetryPromise`/`impRefillPromise`), ручной запасной выход оставлен отдельной оговоркой («если к вечеру не появятся, вставьте дамп заново» — Mac бывает выключен, а пятничный дамп ждёт понедельника). ⚠️ Дыра в реестре региона обещания НЕ получает (`IMP_NO_COURT_RE`): локальная машина пойдёт тем же кодом и отвергнет строку так же — там задача и правда оператору. Маркер `source` подписывает строку журнала «🖥 повтор с локальной машины · время» — иначе обещание было бы непроверяемым. ⚠️ Массив заведений обязан называться `parts` и строиться выражением `["+" + (item.added || 0) + " в картотеку"]` буквально — страж `test_bank_counters_reach_operator` держит эту строку целиком. Плюс секундный тикер ожидания (`impStartTicker`; сети не касается — поллинг реже 30 с нельзя, KV) — после «страница принята» статус стоял немым ровно полминуты. Стражи — [test_admin_pult.py](../../../scripts/tests/test_admin_pult.py), [test_writ_waiver.py](../../../scripts/tests/test_writ_waiver.py), [test_import_search_dump.py](../../../scripts/tests/test_import_search_dump.py).

**Операторская после разбора 09.09.2026 (решения юриста).** Роль живёт только на Урале (~14 операторов под одним секретом, 57 капчёвых судов из 69), и три места врали или молчали именно ему. (1) **Пульт оператора — «Обход карточек» + «В очереди» + «Импорты»** (`renderScanTile`/`renderQueueTile` в admin_page.js). Прежняя «Последний прогон» показывала ран `replay_on_push` (сборка дайджеста после пуша VPS): длительность рендера, «Mac ·» и номер GitHub-рана, куда оператора не пустят — теперь она `data-owner-only`, и `loadGhRuns` у оператора не зовётся вовсе (плюс у владельца «Mac ·» → «сервер ·»: парсит VPS). «Обход карточек» берёт `last_run` из `parse_health.json` (`cards_read_today`/`cards_planned_today`, разбивка `instances`): капчёвые суды СЮДА ВХОДЯТ — у них закрыт только поиск, карточки читает каждый обход, это и есть канал мониторинга оператора; красная при `courts_unavailable>0` или доле <85% (порог `cloud_run_ok --run-complete`), серая «обхода ещё не было», если `at` не сегодня. ⚠ «Сегодня» — сравнением СТРОКИ даты (`localDateStr`), не через `parseIso`: naive-штамп страница читает как UTC (+5 ч). Прежняя «Карточки судов» считалась по последним 50 записям журнала импортов и протухала в обе стороны (провал недельной давности выпадал из окна → зелёная сама; пятничный дамп без карточек держал красный, хотя обход давно читал суд) — удалена, `collectCardTrouble` остался ради пометки «⚠ попытка …» в строках светофора. «В очереди» считает `queued`/`started`/`dispatched` журнала в `renderImportHistory` (тот зовётся и на горячем поллинге `logonly`), красная по `impQueueStale` («сервер не забрал — сообщите владельцу»), подпись — «повтор провалов — сегодня в 14:00» из `next_slot_at`. (2) **Вкладка «Система» — только владельцу** (`data-owner-only` на чипе и секции): «Здоровье парсеров» ведётся по `courts_for_search`, а тот исключает `search_gated` — на Урале 15 источников (12 судов ЯНАО, апелляция ЯНАО, кассация), ни одного суда оператора. Карточка «Закрыть дело — ИЛ не нужен» у оператора переехала третьей на «Импорт» свёрткой `.ww-fold`: тело ОДНО (`WW_BODY`), `WW_CARD` выбирается ролью, в «Системе» рендерится `${isOperator ? "" : WW_CARD}` — ⚠ дубль карточки давал два `id="ww-card"`, и JS заполнял скрытую копию. (3) **Роль-словарь `IMP_T`**: владельцу — правда об инфраструктуре («повторит локальная машина», «проверьте VPS (court-import.timer)», «очередь GitHub»; строки держат стражи импортёра/резерва), оператору — «повтор пройдёт автоматически» / «сообщите владельцу». (4) **Ярлыки корзин «Пропущено»** переписаны словами оператора («N исков банка завершены без решения (возврат / прекращение / без рассмотрения)», «с уже выданным листом», «уже отработали (срок архива вышел)», «отсеяны ранее (повторная строка)», «сверх лимита карточек за импорт — вставьте дамп ещё раз», «где банк — третье лицо», «дел дочерних компаний Сбера»). Стражи — `test_operator_gets_scan_and_queue_tiles`, `test_scan_tile_checks_today_by_date_string`, `test_system_tab_is_owner_only`, `test_operator_texts_have_no_infra_words` (test_admin_pult.py), `test_operator_sees_writ_card_on_import_tab` (test_writ_waiver.py).
...
`dataFileKind` (main/bank) → дебаунс 1,5 с → `applyPendingDataRefresh`:
`loadFromSheet(url,{quiet:true})` и/или `reloadBankDataset()`, повторный fetch
попадает в уже свежий кэш (мгновенно, без сети), проход ТИХИЙ. **Тост «Данные
обновлены» убран 03.09.2026** (решение юриста): с автообновлением при возврате
во вкладку он срабатывал на каждый утренний заход (данные меняются каждые
30 мин с 06:00) и двоился — три файла докачиваются с разницей больше прежних
400 мс дебаунса; свежесть видна по штампу «Данные от: …» в шапке. Пока проход
в полёте (`_dataRefreshInFlight`), сигналы копятся и дочитываются одним
хвостовым проходом (стражи `test_background_refresh_is_silent`,
`test_background_refresh_passes_do_not_overlap`). Пока открыт
...
- **Фронт**: ключи `lsKey('profile_id')`/`profile_base_ts`/`profile_dirty`;
  ...
  связке — ранний return (не воскрешать снимок). UI — раздел «Синхронизация
  устройств» в шторке ⚙ «Настройки» (с 03.09.2026; до того — кнопка 🔗 в
  шапке, капсулы на мобиле ужаты 52→46px: вчетвером не влезали на 320px) →
  кнопка «Связать устройства» открывает прежнюю шторку «Синхронизация
  подписок»; **на ≥769px это мини-окно по центру**
...
- **Профиль-сирота раскрывается (29.08.2026, вопрос юриста «вижу 12 дел без
  push — какие это дела и чьё устройство?»)**: `orphanProfileHtml` —
  свёртка со списком дел (общий `caseRowHtml` с `{readOnly:true}`: у профиля
  нет endpoint'а, и крестик «убрать номер» был бы мёртвым — `handleAction`
  выходит на пустом endpoint), датами создания/правки набора, признаком
  «📅 календарь» и подсказкой «набор пересекается с подпиской «Имя» — N из M»
  (`profileLikelyOwner`, порог 2 совпадения). Набор приезжал в `/admin/data` с
  самого начала — не хватало рендера; Worker добавил лишь `has_feed` +
  `feed_token_created_at` (⚠️ сам `feed_token` наружу НЕ отдаём — bearer-ссылка
  на `.ics`). Поиск теперь находит и сирот (`profileMatches`; прежний `if (!q)`
  прятал их при любом запросе, и номер, который ведёт ТОЛЬКО такой профиль, не
  искался вовсе). ⚠️ **Устройств в профиле нет вовсе** — связь односторонняя
  (`sub.profile_id` → профиль), ни UA, ни истории; «без push-устройств» = либо
  уведомления запрещены (★ синхронизируются и без них), либо `sub:*` истёк по
  60-дневному TTL, либо профиль заведён подпиской на календарь
  (`handleProfileCalendarToken` создаёт его без push). Опознание владельца —
  косвенное: пересечение набора + ⚙ Настройки → Синхронизация на самом
  устройстве (дашборд показывает там первые 8 символов id). Писать UA в профиль осознанно НЕ стали
  (решение 29.08.2026). ⚠️ Раскрытие помнит `profilesOpen` (ключ —
  `data-profile-id`), пишется по КЛИКУ на `summary`, а не по событию `toggle` —
  та же грабля, что у `.sub-card`.
...
- Стражи — [scripts/tests/test_watchlist_profiles.py](../../../scripts/tests/test_watchlist_profiles.py)
  (47 тестов: LWW-контракт, отсутствие push-гардов, ленивый jsqr, цифровой
  код, delivery-freeze, сквозная проводка, раскрытие профиля-сироты).
...
UID, переносы обновляют, отпавшие исчезают). Кнопки — раздел «Календарь
заседаний» шторки ⚙ «Настройки» (`calFeedBlockHtml` в app.js; до 03.09.2026
жил в модалке синка 🔗 в двух её ветках).
...
- Фронт — «один тап» (29.08.2026): в настройках ОДНА умная кнопка
  ...
  на телефоне это главная кнопка панели, на компьютере главная — «Скачать
  файл (.ics)», а подписка уходит во второй ряд; «Скопировать ссылку» всегда
  (обе добывают токен сами), «Перевыпустить» и показ ссылки в свёртке «Как
  это работает» — только при выданном токене. **Корпоративный OWA (03.09.2026):** рабочие станции
  банка — Linux, почта — веб-OWA, настольного Outlook нет; «Добавить
  календарь → Из Интернета» там исполняет сервер Exchange, у которого нет
  выхода в интернет (не открыл даже публичный ICS Google, ошибка «адрес
  введён неправильно» = любая неудача загрузки), со стороны фида не лечится
  (Worker отвечает на GET/HEAD любым UA, TLS в порядке). Единственный путь в
  OWA — «Скачать файл» (`downloadCalFeed`, https-форма ссылки: `webcal:`
  перехватил бы календарь; тип text/calendar браузер сохраняет файлом, Worker
  не менялся) → импорт «Из файла», снимок без обновлений; текст блока говорит
  это прямо. Кнопка «Outlook» (`outlook.live.com/calendar/0/addfromweb` —
  личные ящики Microsoft) УДАЛЕНА — в банке не открывалась, страж
  `test_outlook_button_stays_removed`. Автоматический путь в OWA возможен
  только письмами-приглашениями (METHOD:REQUEST) на рабочую почту — отдельный
  канал, решение после проверки шлюза юристом. `CAL_FEED_TOKEN_KEY` (`lsKey('cal_feed_token')`) — только
...
Разгрузка шапки (предложение юриста): до этого в ней жили до четырёх капсул
(🔗 синк, 🔔 колокольчик push — вставлялся скриптом `injectPushBell`, тема,
«Обновить»), на 320px они жались до 46px. Синк, push, календарь и «Обновить»
— вещи редкие, им место за шестерёнкой; **в шапке остались ⚙ и тема** (ровно
два `<button>`, страж `test_header_has_gear_and_only_two_capsules`).
Форма — та же шторка, что у синка: `#settings-sheet`/`#settings-scrim` в HTML,
`openSettingsSheet`/`closeSettingsSheet`/`renderSettingsSheet` (app.js, рядом с
`closeSyncSheet`), на ≥769px — общее мини-окно `#sync-sheet, #whatsnew-sheet,
#settings-sheet` (styles.css; регексп группы стерегут три теста), в
`uiBusyForRefresh` — та же блокировка фонового обновления. Рендер
конкатенацией, без template literal.

- **Вёрстка — список строк в духе системных настроек** (вторая итерация
  того же дня; первая была «стеной текста» — разбор юриста): строка
  `settingsRowHtml` = значок (`ST_ICONS`, inline-SVG на currentColor; эмодзи
  в кнопках нет — страж) · название · короткий статус (`is-on` зелёный /
  `is-warn` янтарный) · шеврон; тап раскрывает ОДИН раздел
  (`_settingsOpenSection`, `settingsToggle`, аккордеон) с одной главной
  кнопкой `.st-btn-primary`, пояснения — в свёртке `settingsFoldHtml` «Как
  это работает». Строка-действие без панели — `opts.action`
  («Обновить данные»). Классы `.st-*` в styles.css; `.sync-block`/`.cal-*`
  из первой версии убраны.
- **Разделы сверху вниз:** «Уведомления» (`settingsPushSectionHtml` —
  статус словами: `on`/`ready` + кнопка `#st-push-btn` → `settingsPushEnable`/
  `ios-install`/`denied`/`none`; источник — `setPushUiState(state, onReadyClick)`,
  который ЗАМЕНИЛ `injectPushBell` и в шапку ничего не вставляет),
  «Синхронизация устройств» (`settingsSyncSectionHtml` — статус профиля и
  ⏳ неотправленные ★, кнопка → `settingsOpenSync` закрывает настройки и
  открывает прежнюю шторку синка: коды, QR, сканер там без изменений),
  «Календарь заседаний» (`settingsCalendarSectionHtml` → `calFeedBlockHtml`:
  **главная кнопка зависит от устройства** — `calIsMobileDevice()` → телефон
  «Добавить в календарь» (подписка), компьютер «Скачать файл (.ics)» для OWA;
  см. раздел выше), «Обновить данные» (`settingsRefreshRowHtml` —
  строка-действие, статус «Данные от …» из `currentDataStamp`; сюда переехала
  кнопка шапки, спиннер — `#st-item-refresh.is-loading`), «О приложении»
  (`settingsAboutSectionHtml`: версия из `?v=` у app.js, территория из
  `#header-region`, «Показать «Что нового»» → `showWhatsNewAgain` снимает
  `lsKey('whatsnew_seen')`).
- **Автообновление при возврате во вкладку** (`AUTO_REFRESH_MIN_MS`=10 мин,
  слушатель `visibilitychange` рядом с пингом визита): SW освежает
  ...
  кнопка «Обновить». Гейты: есть данные, не офлайн, `uiBusyForRefresh()`
  ложно (drawer/шторки), 10 минут от `_lastDataLoadAt` (ставится в
  `loadFromSheet` на успехе). Сам fetch идёт через SW (stale-while-revalidate):
  свежий снимок доедет сообщением `data-updated` штатным путём.
- **Точка «неотправленные ★»** переехала с `#btn-sync` на `#btn-settings.pending::after`
  (`updateSyncButton`; зелёной подсветки «связано» у шестерёнки нет — статус
  словами в шторке). Подсказки в коде («введите код на втором устройстве…»,
  тост deep-link `?pair=`) ведут в «⚙ Настройки → Синхронизация».
- **Анонс «Что нового»** — `WHATSNEW_ID = 'settings-2026-09'`: переезд
  настроек и «Обновить» (кнопка `whatsNewOpenSettings`) + «Скачать файл» для OWA; прежний
  выпуск 26.08 (`whatsNewShowFreshness`/`whatsNewOpenSync`) удалён вместе с
  разметкой.
- Стражи — [scripts/tests/test_frontend_settings.py](../../../scripts/tests/test_frontend_settings.py),
  обновлены test_calendar_feed / test_watchlist_profiles / test_frontend_whatsnew /
  test_worker_fallback.
...
⚠️ **У раскрытого дайджеста НЕТ потолка высоты** (21.08.2026): базовое правило
`.digest-body` несёт `overflow: hidden`, и стоявший в `.digest-block.expanded`
`max-height: 10000px` молча срезал хвост — на телефоне это ~390 визуальных
строк, у Урала пропадало больше половины выпуска вместе с футером, БЕЗ скролла
и без индикатора. Дефект прятался за beacon-режимом (свежий дайджест
открывается именно там, потолка нет с самого начала) и бил при ПОВТОРНОМ
ручном раскрытии. Теперь `max-height: none` + `overflow: visible`, как в
beacon; цена — раскрытие не анимируется по высоте (переход остался на
opacity/padding). Страж —
[scripts/tests/test_frontend_digest_height.py](../../../scripts/tests/test_frontend_digest_height.py).
