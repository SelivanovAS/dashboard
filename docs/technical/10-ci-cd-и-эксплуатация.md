...
|---------|---------|-----------|
| `--json` | `main_json` ([1971](../../scripts/court_monitor/runs.py#L1971)) | **Основной прогон**: парсинг + JSON + дайджест + рассылка + коммит. Запускается кроном. `--smart-skip` (env `SKIP_NON_WORKING_DAYS`) пропускает нерабочие дни и дела с известной будущей датой. |
| _(без флага)_ | `main` ([1065](../../scripts/court_monitor/runs.py#L1065)) | Legacy CSV-прогон (апелляция). |
| `--digest-only` | `main_digest_only` ([5117](../../scripts/court_monitor/runs.py#L5117)) | Только дайджест по текущим данным, без парсинга. |
| `--replay-last [--push-all]` | `main_replay_last` ([4799](../../scripts/court_monitor/runs.py#L4799)) | Переиграть последний дайджест из `last_digest_context.json` с актуальным промптом. Push — владельцу (или всем при `--push-all`). |
| `--push-last-digest [--owner-only]` | `main_push_last_digest` ([4977](../../scripts/court_monitor/runs.py#L4977)) | Повторно разослать уже сохранённый дайджест. |
| `--backfill-appeal-anchors` | `main_backfill_appeal_anchors` ([1324](../../scripts/court_monitor/runs.py#L1324)) | Разовый бэкфилл якорей УИД/номеров из апел. карточек. |
...
`validate_environment` ([1020](../../scripts/court_monitor/runs.py#L1020)) проверяет
наличие ключей на старте; `check_court_available` ([1052](../../scripts/court_monitor/runs.py#L1052))
