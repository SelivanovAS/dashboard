...
|---------|---------|-----------|
| `--json` | `main_json` ([1738](../../scripts/court_monitor/runs.py#L1738)) | **Основной прогон**: парсинг + JSON + дайджест + рассылка + коммит. Запускается кроном. `--smart-skip` (env `SKIP_NON_WORKING_DAYS`) пропускает нерабочие дни и дела с известной будущей датой. |
| _(без флага)_ | `main` ([955](../../scripts/court_monitor/runs.py#L955)) | Legacy CSV-прогон (апелляция). |
| `--digest-only` | `main_digest_only` ([4325](../../scripts/court_monitor/runs.py#L4325)) | Только дайджест по текущим данным, без парсинга. |
| `--replay-last [--push-all]` | `main_replay_last` ([4009](../../scripts/court_monitor/runs.py#L4009)) | Переиграть последний дайджест из `last_digest_context.json` с актуальным промптом. Push — владельцу (или всем при `--push-all`). |
| `--push-last-digest [--owner-only]` | `main_push_last_digest` ([4186](../../scripts/court_monitor/runs.py#L4186)) | Повторно разослать уже сохранённый дайджест. |
| `--backfill-appeal-anchors` | `main_backfill_appeal_anchors` ([1214](../../scripts/court_monitor/runs.py#L1214)) | Разовый бэкфилл якорей УИД/номеров из апел. карточек. |
...
`validate_environment` ([910](../../scripts/court_monitor/runs.py#L910)) проверяет
наличие ключей на старте; `check_court_available` ([942](../../scripts/court_monitor/runs.py#L942))
