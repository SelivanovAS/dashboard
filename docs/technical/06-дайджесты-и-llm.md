...
([строка 159](../../scripts/court_monitor/config.py#L159)): `claude` по
...
[Строка 682](../../scripts/court_monitor/digest/template.py#L682). Собирает весь HTML дайджеста
...
([596](../../scripts/court_monitor/digest/template.py#L596)).
...
2. Считается ключ кэша (`_act_cache_key`): для Claude — прежний
   `sha1(act_text + "|v2-ratio")[:16]`, для gigachat/openrouter в ключ
...
5. При любой ошибке/пустом ответе → `None`, и вызывающий код откатывается на
   ...
   [542](../../scripts/court_monitor/digest/template.py#L542)).
...
[Строка 871](../../scripts/court_monitor/digest/llm.py#L871). При `DIGEST_POLISH=1`
черновой HTML отправляется в LLM с системным промптом
`_DIGEST_POLISH_SYSTEM_PROMPT` ([730](../../scripts/court_monitor/digest/llm.py#L730)) для
косметики (капитализация, жирные даты, склонения, сокращение длинных категорий).
Результат проходит `_validate_polished_html`
([827](../../scripts/court_monitor/digest/llm.py#L827)): проверяется контракт
...
| `_close_open_tags` ([1334](../../scripts/court_monitor/digest/postprocess.py#L1334)) / `_strip_orphan_close_tags` ([1350](../../scripts/court_monitor/digest/postprocess.py#L1350)) / `truncate_html_message` ([1377](../../scripts/court_monitor/digest/postprocess.py#L1377)) | Закрытие тегов и безопасная обрезка под лимит Telegram. |
...
Целевой объём задаётся `DIGEST_CHAR_LIMIT` ([221](../../scripts/court_monitor/config.py#L221)).
...
> ⚠️ Промпты (`GIGACHAT_SYSTEM_PROMPT` [75](../../scripts/court_monitor/digest/llm.py#L75),
