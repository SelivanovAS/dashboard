...
`scheduled(event, env)` ([worker.js:1453](../../cloudflare-worker/worker.js#L1453)):
...
Маршрутизатор — `fetch(request, env)` ([1496](../../cloudflare-worker/worker.js#L1496)).
...
ему эти файлы вообще не запрашивает. Карточки подписчиков свёрнуты по
