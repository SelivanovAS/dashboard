...
# ── Гейт подстраховки: облако сегодня уже отработало? ────────────────────────
# Агент на Mac — ВТОРОЙ слой после облака. Зрячий облачный прогон уже есть
# (журнал здоровья свежий: git pull строкой выше) → парсить второй раз не
# нужно: обе машины сделали бы одну работу, а push с маркером «(Mac-парсинг)»
# разослал бы дайджест ПОВТОРНО всем подписчикам. Слепой прогон (все источники
# по нулям — адрес раннера заблокирован) или прогона не было — идём парсить.
# СТОИТ ДО маршрутов и пробы судов: при живом облаке агент выходит за секунды
# и не дёргает 🚨-алерт «суд недоступен» из-за собственных сетевых проблем.
# --force (пульт, ручной запуск юриста) гейт пропускает; --check ниже
...
if [ "$CHECK_ONLY" != "1" ] && [ "$FORCE" != "1" ]; then
  if CLOUD_STATUS=$("$PYTHON" ops/mac-local-run/cloud_run_ok.py --report 2>/dev/null); then
    log "Облако сегодня уже отработало ($CLOUD_STATUS) — пропуск, Mac не нужен"
    finish_pusher
    exit 0
  fi
  log "Облако не справилось (${CLOUD_STATUS:-статус не прочитался}) — парсим на Mac"
fi
...
# (08:00–11:00), и сорвавшуюся пробу добивает следующий (20.08.2026 Урал не
# прошёл в 08:19 и спарсился в 08:30), а алерт на каждую неудачу дал 5
# одинаковых сообщений за утро. Тихий выход до 10:55; в/после 10:55 (слот
# 11:00 — последний, либо Mac проснулся после сна) — ОДИН алерт в день
# (маркер-файл .alerted-parse-ДАТА, живёт рядом с логами, в git не попадает).
# Ручной запуск (--force) кричит сразу — юрист смотрит на экран.
PROBE_ALERT_AFTER_MIN=655   # 10:55 местного
probe_failed() {  # $1 = диагностика канареек; наружу не возвращается
  local diag="$1" now marker
  ...
  if [ "$now" -ge "$PROBE_ALERT_AFTER_MIN" ] && [ ! -f "$marker" ]; then
    rm -f "$LOG_DIR"/.alerted-parse-* 2>/dev/null
    : > "$marker"
    # Статус гейта называет, ЧТО именно не удалось за утро: «поиски слепые
    # (мониторинг дел мог пройти)» ≠ «N карточек срезано» ≠ «прогона не было».
    # Без него алерт «так и не спарсилось» пугал при доехавшем дайджесте
    # (20.08.2026, Урал). CLOUD_STATUS захвачен гейтом выше; --force сюда не
    # доходит (кричит сразу), поэтому пустым он бывает только теоретически.
    alert_telegram "за утро не дочиталось (${CLOUD_STATUS:-статус гейта не прочитался}); суды не отвечают ($diag)"
    # «ERROR:» в начале — финальная строка для progress_pusher (END_RE).
    log "ERROR: за утро не дочиталось (${CLOUD_STATUS:-?}) — суды не отвечают (алерт отправлен, один в день)"
    finish_pusher
    exit 1
  fi
